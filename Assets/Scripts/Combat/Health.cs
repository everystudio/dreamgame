...
public class Health : MonoBehaviour
{
    ...
    protected virtual void Initialize()
    {
        ...
        collider = this.gameObject.GetComponentInChildren<Collider2D>();
    ...
    public void Damage(float _amount, Vector3 _hitlocation, GameObject _cause)
    {
        ...
        if (!isDead)
        {
            ...
            {
                iDamageableInterfaces[i].OnDamaged(
                    new DamageInfo(
                        minHP,
                        maxHP,
                        _hitlocation,
                        (_cause != null) ? _cause.transform.position : Vector3.zero,
                        (_cause != null) ? _cause.name : "",
                        _amount));
                //iDamageableInterfaces[i].OnDamaged((int)_amount);
            }
        }
